...
//import gnu.trove.map.TIntHashSet;
...
public class Dirichlet {
    ...
    /**
     * Constructor that takes an alphabet representing the meaning of each
     * dimension
     */
    ...
    /**
     * A symmetric Dirichlet with alpha_i = 1.0 and the number of dimensions of
     * the given alphabet.
     */
...
//		Normalize
    ...
    /**
     * Dirichlet-multinomial: draw a distribution from the dirichlet, then draw
     * n samples from that multinomial.
     */
     ...
     *
     * @param n The <i>expected</i> total number of counts in the returned
     * vector. The actual number is ~ Poisson(<code>n</code>)
     */
...
//		goes berzerk for lambda above ~500.
    ...
    /**
     * Create a set of d draws from a dirichlet-multinomial, each with an
    ...
    /**
     * This directly calculates the difference between two log gamma functions
     * using a recursive formula. The break-even with the Stirling approximation
     * is about n=2, so it's not necessarily worth using this.
     */
     ...
     * <code>z</code> approaches 0. If <code>z</code> is less than 2, we shift
     * it up, calculate the approximation, and then shift the answer back down.
     */
...
//		if (z < 0) { System.out.println(" less than zero"); }
    ...
    /**
     * Learn the concentration parameter of a symmetric Dirichlet using
     * frequency histograms. Since all parameters are the same, we only need to
     * keep track of the number of observation/dimension pairs with count N
     *
     * @param countHistogram An array of frequencies. If the matrix X represents
     * observations such that x<sub>dt</sub> is how many times word t occurs in
     * document d, <code>countHistogram[3]</code> is the total number of cells
     * <i>in any column</i> that equal 3.
     * @param observationLengths A histogram of sample lengths, for example
     * <code>observationLengths[20]</code> could be the number of documents that
     * are exactly 20 tokens long.
     ...
     *
     * @param parameters A reference to the current values of the parameters,
     * which will be updated in place
     * @param observations An array of count histograms.
     * <code>observations[10][3]</code> could be the number of documents that
     * contain exactly 3 tokens of word type 10.
     * @param observationLengths A histogram of sample lengths, for example
     * <code>observationLengths[20]</code> could be the number of documents that
     * are exactly 20 tokens long.
    ...
    public static double learnParameters(double[] parameters,
        ...
        for (int iteration = 0; iteration < numIterations; iteration++) {
            ...
            for (k = 0; k < parameters.length; k++) {
                ...
                if (parameters[k] == 0) //not used component (ignore it)
                {
                    continue;
                }
...
//		System.out.println(distributionToString(magnitude, partition));
    ...
    /**
     * Estimate a dirichlet with the moment matching method described by
     * Ronning.
     */
...
//		Trove hashmaps, but with fixed size arrays.
    ...
    /**
     * Similar to the Dirichlet-multinomial test,s this is a likelihood ratio
     * based on the Ewens Sampling Formula, which can be considered the
     * distribution of partitions of integers generated by the Chinese
     * restaurant process.
     */
