...
public class FeatureSequence implements Sequence, Serializable, AlphabetCarrying {
     ...
     *
     * @param dict A dictionary that maps objects in the sequence to numeric
     * indices.
     * @param features An array where features[i] gives the index in dict of the
     * ith element of the sequence.
     */
     ...
     * <code>cutoff</code> times in the corpus, as indicated by the provided
     * counts and also do not exist in minAlphabet (helping in creating
     * incremental vocabularies). Also swap in the new, reduced alphabet. This
     * method alters the instance in place; it is not appropriate if the
     * original instance will be needed.
     */
    public void prune(double[] counts, Alphabet newAlphabet,
            int cutoff, Alphabet minAlphabet) {
        ...
        int newIndex = 0;
        for (int i = 0; i < length; i++) {
            if (counts[features[i]] >= cutoff || minAlphabet.lookupIndex(features[i]) != -1) {
     ...
     * Remove features from the sequence that occur fewer than
     * <code>cutoff</code> times in the corpus, as indicated by the provided
     * counts. Also swap in the new, reduced alphabet. This method alters the
     * instance in place; it is not appropriate if the original instance will be
     * needed.
     */
    public void prune(double[] counts, Alphabet newAlphabet,
            int cutoff) {
