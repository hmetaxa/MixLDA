...
/**
 * Hierarchical PAM, where each node in the DAG has a distribution over all
 * topics on the next level and one additional "node-specific" topic.
 *
 * @author David Mimno
 */
public class HierarchicalPAM {
    ...
    static CommandOption.String inputFile = new CommandOption.String(HierarchicalPAM.class, "input", "FILENAME", true, null,
            "The filename from which to read the list of training instances.  Use - for stdin.  "
            + "The instances must be FeatureSequence or FeatureSequenceWithBigrams, not FeatureVector", null);

    static CommandOption.String stateFile = new CommandOption.String(HierarchicalPAM.class, "output-state", "FILENAME", true, null,
            "The filename in which to write the Gibbs sampling state after at the end of the iterations.  "
            + "By default this is null, indicating that no file will be written.", null);

    static CommandOption.Double superTopicBalanceOption = new CommandOption.Double(HierarchicalPAM.class, "super-topic-balance", "DECIMAL", true, 1.0,
            "Weight (in \"words\") of the shared distribution over super-topics, relative to the document-specific distribution", null);

    static CommandOption.Double subTopicBalanceOption = new CommandOption.Double(HierarchicalPAM.class, "sub-topic-balance", "DECIMAL", true, 1.0,
    ...
    static CommandOption.Integer numSuperTopicsOption = new CommandOption.Integer(HierarchicalPAM.class, "num-super-topics", "INTEGER", true, 10,
            "The number of super-topics", null);

    static CommandOption.Integer numSubTopicsOption = new CommandOption.Integer(HierarchicalPAM.class, "num-sub-topics", "INTEGER", true, 20,
    ...
    // Document frequencies used for "minimal path" hierarchical Dirichlets (Num of tables under minimal path )
    ...
    public void printState(PrintWriter out) {
        ...
        for (int doc = 0; doc < superTopics.length; doc++) {
            ...
            for (int position = 0; position < superTopics[doc].length; position++) {
                ...
                output.append(' ');
                output.append(superTopics[doc][position]);
                output.append(' ');
                output.append(subTopics[doc][position]);
                output.append("\n");
            }
