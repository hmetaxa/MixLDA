 ...
 * <p>
 * This class supports a simpler method that makes two passes over the data: one
 * to collect statistics and create an augmented "stop list", and a second to
...
public class FeatureDocFreqPipe extends Pipe {
     ...
     *
     * @param docFrequencyMaxCutoff Remove words that occur in greater than this
     * proportion of documents. 0.05 corresponds to IDF >= 3, 0--> ignore.
     * @param docFrequencyMinCutoff Remove words that occur in smaller than this
     * proportion of documents. 1--> ignore.
     */
    public void addPrunedWordsToStoplist(SimpleTokenizer tokenizer, double docFrequencyMaxCutoff, double docFrequencyMinCutoff) {
        Alphabet currentAlphabet = getDataAlphabet();

        for (int feature = 0; feature < currentAlphabet.size(); feature++) {
            if ((double) counter.get(feature) / numInstances > docFrequencyMaxCutoff || (double) counter.get(feature) / numInstances < docFrequencyMinCutoff) {
     ...
     * Add all pruned words to the internal stoplist of a SimpleTokenizer.
     *
     * @param docFrequencyCutoff Remove words that occur in greater than this
     * proportion of documents. 0.05 corresponds to IDF >= 3.
     */
