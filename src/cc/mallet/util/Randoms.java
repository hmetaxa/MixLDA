...
//	protected double lmss = 0;
	 ...
	 * @return
	 */
	public synchronized double[] nextStirling(int nn) {
            if (allss == null)
                allss = new double[MAXSTIRLING][];
	 ...
	 * @return
	 */
	public synchronized int nextAntoniak(double alpha, int n) {
		double[] p = nextStirling(n);
		double aa = 1;
		...
		// }
		// Vectors.mult(p, 1. / R);
		return nextDiscrete(p) + 1;
	}

    /**
     * draw a Bernoulli sample. Omiros 
     *
     * @param p success probability
     * @return 1 if successful, 0 otherwise
     */
    public synchronized int nextBernoulli(double p) {
        double a = nextDouble();
        if (a < p) {
            return 1;
        }
  ...
  // generate Gamma(1,1)
    ...
    /**
     * Wrap this instance as a java random, so it can be passed to legacy
     * methods. All methods of the returned java.util.Random object will affect
     * the state of this object, as well.
     */
